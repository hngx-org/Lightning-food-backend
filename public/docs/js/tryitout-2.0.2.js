...
function makeAPICall(method, path, body, query, headers, endpointId) {
  ...
  return fetch(url, {
    ...
    Promise.all([response.status, response.text(), response.headers]),
  );
}
...
function handleError(endpointId, err) {
  ...
  document.querySelector(`#execution-error-message-${endpointId}`).textContent =
    errorMessage;
...
async function executeTryOut(endpointId, form) {
  ...
  if (form.dataset.authed === '1') {
    ...
    if (authHeaderEl)
      headers[authHeaderEl.name] =
        authHeaderEl.dataset.prefix + authHeaderEl.value;
  }
